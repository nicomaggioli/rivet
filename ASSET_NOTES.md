...
Rivet is the founder-selected company name. The angular R logo was supplied by the founder and faithfully traced as SVG. The wordmark “rivet” is Geist at weight 460, converted to outlines: 48 units per em on a baseline at y 49, starting at x 69 beside the mark, with −1.9 units of tracking and no kerning.
...
The structural-detail and studio-worktable photographs are generated illustrative brand imagery. They do not depict customer projects.
...
The trailer (0:40) follows one pursuit from a federal deadline to an assembled SF330. Screens are shown as floating cards cut from the same captures, never redrawn: UI elements that appear during a shot (pipeline cards, the fit score, checks, matrix dots, GENERATED chips) are revealed from the capture's own pixels, and the fit score counts up in the dashboard's own font. The music bed is an Eleven Music instrumental, edited on its bars to the narration (a stop before “Rivet”, a second drop on “Your whole SF330”, a final hit on the last “Rivet”) and ducked under the voice; the light sound effects are synthesized.

The narration is ElevenLabs Roger on Eleven v3 at Natural stability. The trailer (0:40 with its end card) is one take of the script written out as natural speech, with no audio tags, closing on “Rivet. From SAM.gov, to a winning proposal!”. The tour (2:28) is one take whose closing paragraph comes from a second take of the same read, because the first clipped its last word; the two are joined in the paragraph pause and level-matched. Each is loudness-normalised to −16 LUFS, with a short lead-in (0.35 s for the trailer, 0.5 s for the tour); the trailer's final mix with music and effects is also −16 LUFS. Word times are refined against the audio, so cuts, highlights and captions land on the spoken words.
